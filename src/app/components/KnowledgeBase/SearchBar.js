...
export default function SearchBar() {
  ...
  const handleSearch = (e) => {
    e.preventDefault();
    if (searchTerm.trim()) {
      router.push(`/knowledgebase/search?q=${encodeURIComponent(searchTerm)}`);
    }
  };

  const handlePopularSearch = (term) => {
    setSearchTerm(term);
    router.push(`/knowledgebase/search?q=${encodeURIComponent(term)}`);
  };
  ...
  return (
    <div className="max-w-2xl mx-auto animate-fadeIn">
      ...
      <div className="mt-3 text-sm text-gray-600">
        ...
        <button 
          onClick={() => handlePopularSearch('email setup')} 
          className="text-blue-600 cursor-pointer hover:underline"
        >
          email setup
        </button>, 
        <button 
          onClick={() => handlePopularSearch('domain transfer')} 
          className="text-blue-600 cursor-pointer hover:underline"
        >
          domain transfer
        </button>
